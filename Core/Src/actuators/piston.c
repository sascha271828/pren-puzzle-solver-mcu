...
static void Piston_SetExtend(void) {
  ...
  HAL_GPIO_WritePin(piston.piston_retract.port, piston.piston_retract.pin, GPIO_PIN_SET);
}
static void Piston_SetRetract(void) {
  pwm_extending = false;
  piston.is_moving = true;
  HAL_GPIO_WritePin(piston.piston_extend.port, piston.piston_extend.pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(piston.piston_retract.port, piston.piston_retract.pin, GPIO_PIN_RESET);
}
