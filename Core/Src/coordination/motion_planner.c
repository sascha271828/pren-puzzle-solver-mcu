...
static MoveBlock_t PlanMoveToAbsoluteMM(float absolute_x_mm,
  ...
  int32_t target_steps_x = (int32_t)(absolute_x_mm * STEPS_PER_MM_X);
  int32_t target_steps_y = (int32_t)(absolute_y_mm * STEPS_PER_MM_Y);
...
MoveBlock_t MotionPlanner_PlanMoveToPickMM(float pick_x_mm, float pick_y_mm) {
  float abs_x = pick_x_mm + OFFSET_PICK_X_MM;
  float abs_y = pick_y_mm + OFFSET_PICK_Y_MM;
...
MoveBlock_t MotionPlanner_PlanMoveToPlaceMM(float place_x_mm,
                                            float place_y_mm) {
  float abs_x = place_x_mm + OFFSET_PLACE_X_MM;
  float abs_y = place_y_mm + OFFSET_PLACE_Y_MM;
